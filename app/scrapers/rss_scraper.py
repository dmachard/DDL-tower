        ...
        async with async_playwright() as p:
            ...
            for item in items_to_process:
                ...
                try:
                        ...
                        if isinstance(self.required_keywords, dict):
                            for kw, tag in self.required_keywords.items():
                                try:
                                    ...
                                    print(f"[RSS_SCRAPER] [{self.name}] Regex error for '{kw}': {e}")
                        else:
                    ...
                    if found_links:
                        # Filter out None from tags and join them
                        tags = [t for t in matched_tags if t]
                        yield {"links": found_links, "override_filename": rss_title, "source_url": url, "tags": tags}
